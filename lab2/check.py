...
base_dir = "lab2\matrix&muls&results"
...
with open('lab2/trust_me.txt', 'w') as f:
    for matrix_dir in os.listdir(base_dir):
        if os.path.isdir(os.path.join(base_dir, matrix_dir)):
            ...
            for thread_dir in os.listdir(os.path.join(base_dir, matrix_dir)):
                if os.path.isdir(os.path.join(base_dir, matrix_dir, thread_dir)) and thread_dir.startswith("thread_"):
